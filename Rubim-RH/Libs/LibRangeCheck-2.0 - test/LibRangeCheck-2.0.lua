...
local function getNumSpells()
    local _, _, offset, numSpells = C_SpellBook.GetSpellBookSkillLineInfo(C_SpellBook.GetNumSpellBookSkillLines())
...
local function findSpellIdx(spellName)
    ...
    for i = 1, getNumSpells() do
        local spell, rank = C_SpellBook.GetSpellBookItemName(i, BOOKTYPE_SPELL)
...
local function createCheckerList(spellList, itemList, interactList)
    ...
    if spellList then
        for i = 1, #spellList do
            ...
            local name, _, _, _, minRange, range = C_Spell.GetSpellInfo(sid)
...
function lib:init(forced)
    ...
    if not minRangeCheck then
        ...
        if playerClass == "WARRIOR" then
            ...
            local name = C_Spell.GetSpellInfo(5246) -- ["Intimidating Shout"]
            local spellIdx = findSpellIdx(name)
            if spellIdx then
                minRangeCheck = function(unit)
        ...
        elseif playerClass == "ROGUE" then
            ...
            local name = C_Spell.GetSpellInfo(2094) -- ["Blind"]
            local spellIdx = findSpellIdx(name)
            if spellIdx then
                minRangeCheck = function(unit)
                    return (IsSpellInRange(spellIdx, BOOKTYPE_SPELL, unit) == 1)
