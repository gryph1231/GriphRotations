...
local function APL()
    ...
    if S.lustAT:ID() == RubimRH.queuedSpell[1]:ID() and not Player:Debuff(S.lust1) and not Player:Debuff(S.lust2) and
        not Player:Debuff(S.lust3) and not Player:Debuff(S.lust4) and not Player:Debuff(S.lust5) then
    ...
    if Player:CanAttack(Target) and Target:AffectingCombat() or (Player:AffectingCombat() or Target:IsDummy()) then
        ...
        if S.LivingFlame:IsCastable() and Target:IsInRange(25) then
            return S.LivingFlame:Cast()
        end
