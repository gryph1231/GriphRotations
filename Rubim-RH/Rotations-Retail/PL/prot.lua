...
local function APL()
  ...
  targetRange30 = IsItemInRange(835, "target")
            ...
            if IsReady("Lay on Hands")  and Player:AffectingCombat() and Player:HealthPercentage() < 20 and S.DivineShield:CooldownRemains() > Player:GCD() and not Player:Debuff(S.Forbearance)
            ...
            if IsReady("Divine Shield") and not Player:Debuff(S.Forbearance) and S.FinalStand:IsAvailable() and useDS  then
            return S.DivineShield:Cast()
            end
            ...
            if (not IsEncounterInProgress(Boss) or level <= highkey)  then
                if IsReady("Divine Shield") and not Player:Debuff(S.Forbearance) and S.FinalStand:IsAvailable() 
                and inRange30 >= 1
  ...
  if IsReady("Avenger's Shield")  and targetRange30 and (inRange30 > 2 or Player:Buff(S.MomentofGloryBuff)) then
    return S.AvengersShield:Cast()
end
 ...
 if IsReady("Consecration") and (targetRange8 and not Player:IsMoving() or targetRange5 and  Player:IsMoving()) and (Player:BuffStack(S.SanctificationBuff) == 5 or not AuraUtil.FindAuraByName("Consecration", "player")) then
  ...
  if IsReady("Judgment") and targetRange30 and (not Player:Buff(S.SanctificationEmpowerBuff) and tierequipped()>=2) then
    return S.Judgment:Cast()
end
  -- hammer_of_wrath
  ...
  if IsReady("Consecration") and  (targetRange8 and not Player:IsMoving() or targetRange5 and  Player:IsMoving()) and (not Player:Buff(S.ConsecrationBuff) and (Player:BuffStack(S.SanctificationBuff) ~= 5 or tierequipped()<2)) then
    return S.Consecration:Cast()
end
  ...
  if IsReady("Word of Glory")  and (Player:Buff(S.ShiningLightFreeBuff)) and  Player:HealthPercentage() <= 80 then
            return S.WordofGlory:Cast()
            end


  -- consecration,if=!buff.sanctification_empower.up
  if IsReady("Consecration") and (targetRange8 and not Player:IsMoving() or targetRange5 and  Player:IsMoving()) and (not Player:Buff(S.SanctificationEmpowerBuff)) then
    return S.Consecration:Cast()
end
