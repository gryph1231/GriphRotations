...
local function APL()
    ...
    if not AuraUtil.FindAuraByName("Stealth", "player") and not Player:Buff(S.VanishBuff) then
        stealthbasic = false
    else
        stealthbasic = true
    end

    if not AuraUtil.FindAuraByName("Stealth", "player") and not Player:Buff(S.VanishBuff) 
    ...
    if ((stealthbasic or Player:Buff(S.Shadowmeld)) and (Target:AffectingCombat() or target_is_dummy() and Player:AffectingCombat() or Player:Buff(S.VanishBuff))) then
        ...
        if S.ColdBlood:IsCastable() and not Player:BuffP(S.ColdBlood) and Target:IsInRange(8) and finishcondition and Player:CanAttack(Target) then
        ...
        if S.Ambush:IsCastable() and not finishcondition and Target:IsInRange(8) and Player:CanAttack(Target) and (stealthedcto or stealthbasic and S.FindWeakness:IsAvailable() and not Target:Debuff(S.FindWeaknessDebuff) or S.HiddenOpportunity:IsAvailable()) then
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------------------
    if (Target:AffectingCombat() or target_is_dummy() and Player:AffectingCombat()) or Player:AffectingCombat() and not AuraUtil.FindAuraByName("Stealth", "player") then
        ...
        if S.Evasion:IsCastable() and (Cache.EnemiesCount[10] >= 1 or Target:IsInRange(10)) and IsTanking and (Player:HealthPercentage() < 55 or HPpercentloss > 10 and Player:HealthPercentage() < 65) then
            return S.Evasion:Cast()
        end


        if RubimRH.CDsON() and Target:IsInRange(5) and not Target:IsDeadOrGhost() and Player:CanAttack(Target) and Player:AffectingCombat() and not AuraUtil.FindAuraByName("Stealth", "player") then
    ...
    if RubimRH.CDsON() and Target:IsInRange(10) and Player:CanAttack(Target) and Player:AffectingCombat() then
        if S.AdrenalineRush:IsCastable() and not AuraUtil.FindAuraByName("Stealth", "player") and not Player:BuffP(S.AdrenalineRush)
            and (not S.ImprovedAdrenalineRush:IsAvailable() or Player:ComboPoints() <= 2) then
            return S.AdrenalineRush:Cast()
        end


        if S.BladeFlurry:IsCastable() and not AuraUtil.FindAuraByName("Stealth", "player") and ((RubimRH.AoEON() and (not Player:BuffP(S.BladeFlurry)
        ...
        and (EnergyTimeToMaxRounded() > (4 + num(AuraUtil.FindAuraByName("Stealth", "player")) - (Cache.EnemiesCount[bfrange] / 3)) or Cache.EnemiesCount[bfrange]>3 and Player:EnergyDeficit()>50) 
    ...
    -- &combo_points<=2&(!talent.marked_for_death|!cooldown.marked_for_death.ready)&target.time_to_die>=10
        if S.Dreadblades:IsCastable() and Target:IsInRange(8) and (not (stealthedcto or stealthbasic or S.HiddenOpportunity:IsAvailable() and stealthall) 
    ...
    -----------------------------------------------------------------Stealth CTO----------------------------------------------------------------------------
    --------------------------------------------------------------------------------------------------------------------------------------------------------
    if stealthedcto and not AuraUtil.FindAuraByName("Stealth", "player") and (Target:AffectingCombat() or target_is_dummy() and Player:AffectingCombat() or Player:Buff(S.VanishBuff)) then
        if S.BladeFlurry:IsCastable() and ((RubimRH.AoEON() and (not Player:BuffP(S.BladeFlurry) or Player:BuffRemainsP(S.BladeFlurry) < Player:GCD()) and Cache.EnemiesCount[bfrange] >= 2)) then
            return S.BladeFlurry:Cast()
        end
        ...
        if Player:CanAttack(Target) and Target:IsInRange(8) then
            ...
            if S.Ambush:IsCastable() and (stealthedcto or stealthbasic and S.FindWeakness:IsAvailable() 
    ...
    -----------------------------------------------------------------Builders-------------------------------------------------------------------------------
    --------------------------------------------------------------------------------------------------------------------------------------------------------
    if not RubimRH.LastCast(S.MarkedforDeath, 1) and (Target:AffectingCombat() or target_is_dummy() and Player:AffectingCombat()
        ...
        and Player:CanAttack(Target) and Player:AffectingCombat() and not finishcondition then
        ...
        -- and (Player:Buff(S.AudacityBuff) or Player:CanAttack(Target)) then
        --     return S.Ambush:Cast()
        -- end
    ...
    --------------------------------------------------------------------OOR---------------------------------------------------------------------------------
    --------------------------------------------------------------------------------------------------------------------------------------------------------
    if not Target:IsInRange(bfrange) and not Target:Debuff(S.Blind) and not AuraUtil.FindAuraByName("Stealth", "player") and Player:AffectingCombat() and not Player:Buff(S.VanishBuff) and (Target:AffectingCombat() or target_is_dummy() and Player:AffectingCombat() or Player:Buff(S.VanishBuff)) then
