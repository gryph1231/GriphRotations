...
local RtB_BuffsList = {
  ...
  local function RtB_Reroll()
    if not Cache.APLVar.RtB_Reroll then
        ...
        --   variable,name=rtb_reroll,if=talent.hidden_opportunity,value=!rtb_buffs.will_lose.skull_and_crossbones
        --   &(rtb_buffs.will_lose-rtb_buffs.will_lose.grand_melee)<2&buff.shadow_dance.down&buff.subterfuge.down
        --   With Hidden Opportunity, prioritize rerolling for Skull and Crossbones over everything else
        
          if (not Player:Buff(S.SkullandCrossbones) or Player:BuffRemains(S.SkullandCrossbones) > RubimRH.RtBRemains())
            and not Player:Buff(S.SubterfugeBuff) and not Player:Buff(S.ShadowDanceBuff)
            and ((Cache.APLVar.RtB_Buffs.Normal + Cache.APLVar.RtB_Buffs.Shorter) -
              num(Player:Buff(S.GrandMelee) and Player:BuffRemains(S.GrandMelee) <= RubimRH.RtBRemains())) < 2 then
        ...
        -- variable,name=rtb_reroll,if=!talent.hidden_opportunity,value=rtb_buffs<2
        -- &(!buff.broadside.up&(!talent.fan_the_hammer|!buff.skull_and_crossbones.up)
        ...
        if RtB_Buffs() <2 and (not Player:Buff(S.Broadside) and (not S.FantheHammer:IsAvailable() or not Player:Buff(S.SkullandCrossbones))
        and not Player:Buff(S.TrueBearing) or Player:Buff(S.LoadedDiceBuff)) 
        or RtB_Buffs()==2
        and (Player:Buff(S.BuriedTreasure) and Player:Buff(S.GrandMelee) or not Player:Buff(S.Broadside) 
        and not Player:Buff(S.TrueBearing) and Player:Buff(S.LoadedDiceBuff))
        then
            Cache.APLVar.RtB_Reroll = true
        end


        --   variable,name=rtb_reroll,if=!talent.hidden_opportunity&(talent.keep_it_rolling|talent.count_the_odds),
        --   value=variable.rtb_reroll|((rtb_buffs.normal=0&rtb_buffs.longer>=1)&!(buff.broadside.up&buff.true_bearing.up&buff.skull_and_crossbones.up)
        --   &!(buff.broadside.remains>39|buff.true_bearing.remains>39|buff.ruthless_precision.remains>39|buff.skull_and_crossbones.remains>39))
...
if true then
	...
	ambushcondition = (Player:ComboPointsDeficit() >= (2 + (num(S.ImprovedAmbush)) + (num(Player:Buff(S.Broadside)))) and Player:Energy() >= 50 and (not S.CounttheOdds:IsAvailable() or RubimRH.RtBRemains() >= 10))
...
if (AuraUtil.FindAuraByName("Enrage", "target")  and RubimRH.InterruptsON() and S.Shiv:IsCastableQueue(8) and Player:AffectingCombat() and TargetTTD() > 4) then
	return S.Shiv:Cast()
end
...
if not Player:AffectingCombat() and not Player:BuffP(S.VanishBuff) then
    ...
    if S.RolltheBones:IsCastable() and (IsResting("player") == false or Player:CanAttack(Target)) and Cache.EnemiesCount[20] >= 1 
    and ((not Player:BuffP(S.mantle) or Player:Buff(S.Stealth)) and not Player:DebuffP(S.Dreadblades) 
...
and not Player:BuffP(S.ShadowDanceBuff) and not Player:BuffP(S.mantle) and not Player:DebuffP(S.Dreadblades) 
and (RtB_Buffs() == 0 or RtB_Reroll()) then
	return S.RolltheBones:Cast()
end
...
and Target:IsInRange(bfrange) and Player:CanAttack(Target) and Player:GCDRemains() <= 0.5 and Player:AffectingCombat() then
    
	if S.Vanish:IsCastableQueue() and S.FindWeakness:IsAvailable() and not S.Audacity:IsAvailable() 
    and not Target:Debuff(S.FindWeaknessDebuff) and ambushcondition and vanishcondition then
		return S.Vanish:Cast()
    end

    if S.Vanish:IsCastableQueue() and S.HiddenOpportunity:IsAvailable() and not Player:Buff(S.AudacityBuff) 
    and (vanishopportunitycondition or Player:BuffStack(S.Opportunity) < 6) and ambushcondition and vanishcondition then
		return S.Vanish:Cast()
    end
...
if finishcondition then
	if S.BetweentheEyes:IsCastableQueue(20) and UnitName('target') ~= 'Spiteful Shade' and TargetTTD() > 3 and not Target:Debuff(S.Blind) and not Player:Buff(S.Stealth) and Player:AffectingCombat() and not Player:Buff(S.VanishBuff) 
