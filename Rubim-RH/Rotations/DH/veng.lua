...
local function APL()
    ...
    Brand = function()
        ...
        if S.FieryBrand:IsReady('Melee') and Player:CanAttack(Target) then
...
if Player:AffectingCombat() then
        ...
        -- concentrated_flame
        if S.ConcentratedFlame:IsReady() and Player:CanAttack(Target) and Player:AffectingCombat() then
            return S.HoA:Cast()
        end
        -- ripple_in_space
	...
	if S.SigilofSilence:IsReady() and Cache.EnemiesCount[30] > 1 and RubimRH.InterruptsON() and Target:IsInterruptible() and Target:TimeToDie() >= 6 then
        return S.SigilofSilence:Cast()
    end
	
	if S.SigilofMisery:IsReady() and Cache.EnemiesCount[30] > 1  and RubimRH.InterruptsON() and Target:IsInterruptible() and Target:TimeToDie() >= 6 then
