...
if not Player:AffectingCombat() and not Player:IsChanneling(S.EssenceFont) and not IsResting() then
	...
	if LowestAlly("ALL", "HP") <= 85 then
		if S.Vivify:IsCastable() and Player:BuffP(S.InstantVivify) then
	...
	if LowestAlly("ALL", "HP") <= 75 then
		if S.EnvelopingMist:IsCastable() and not Player:IsMoving() then
...
if RubimRH.InterruptsON() then
	...
	if S.Detox:IsCastable() and LowestAlly("ALL", "HP") >= 35 then
...
if LowestAlly("ALL", "HP") <= 20 and (inInstance or Player:AffectingCombat()) then
	if S.LifeCocoon:IsCastable() then
...
and Player:AffectingCombat() and Player:BuffRemainsP(S.AncientTeachings) <= Player:GCD() then
	return S.EssenceFont:Cast()
end

if S.RisingSunKick:IsReady(8) and Player:CanAttack(Target) and Player:AffectingCombat() and Player:BuffP(S.AncientTeachings) then
...
if S.SpinningCraneKick:IsCastable() and Player:AffectingCombat() and Cache.EnemiesCount[10] >= 3
