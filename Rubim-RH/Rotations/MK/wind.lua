		...
		for i=1,40 do
			...
			if UnitExists("nameplate"..i) then           
				...
				if npc_id ~='120651' and npc_id ~='161895' then
					if UnitCanAttack("player",unitID) and IsActionInRange(range_check,unitID) and UnitHealthMax(unitID) > 5 then
...
local function MotCCounter()
    ...
    if UnitCanAttack("player", unitID) and IsActionInRange(38,unitID) and count then
...
local function SCKMax()
    ...
    local Count = MotCCounter()
    if (RangeCount(40) == Count or Count >= 5) then return true; end
    return false
end
...
local function BDBSetup()
    ...
    if S.BonedustBrew:IsReady() and TargetinRange(37) and (SCKMax() and Player:Chi() >= 4) then
    ...
    if S.BlackoutKick:IsReady() and TargetinRange(37) and
    ...
    if S.RisingSunKick:IsReady() and TargetinRange(37) and (ComboStrike(S.RisingSunKick) and Player:Chi() >= 5) then
    ...
    if S.TigerPalm:IsReady() and (TargetinRange(37) and S.SkyReach:IsAvailable() or TargetinRange(37)) and
        (ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= 2) and
        (TargetinRange(37) and S.SkyReach:IsAvailable() or TargetinRange(37)) then
        return S.TigerPalm:Cast()
    end
    -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains,if=combo_strike&active_enemies>=2
    if S.RisingSunKick:IsReady() and TargetinRange(37) and (ComboStrike(S.RisingSunKick) and RangeCount(40) >= 2) then
        return S.RisingSunKick:Cast()
    end
end
...
local function CDSerenity()
    ...
    if S.SummonWhiteTigerStatue:IsReady() and (XuenActive) and TargetinRange(37) then
    ...
    if S.InvokeXuenTheWhiteTiger:IsReady() and TargetinRange(37) and
        (
    ...
    if S.BonedustBrew:IsReady() and TargetinRange(37) and
        (
    ...
    if S.Serenity:IsReady() and TargetinRange(37) and
        (
    ...
    if S.TouchofDeath:IsReady() and TargetinRange(37) and
        (
    ...
    -- if S.TouchofKarma:IsReady() and TargetinRange(37) and RubimRH.CDsON() and
    --     (Target:TimeToDie() > 90 or Target:TimeToDie() < 10) then
    --     return S.TouchofDeath:Cast()
    -- end
    if (Player:Buff(S.SerenityBuff) or Target:TimeToDie() < 20) and TargetinRange(37) then
    ...
    -- lights_judgment
    if S.LightsJudgment:IsReady() and TargetinRange(37) then
        return S.LightsJudgment:Cast()
    end
    -- use_items
    if RubimRH.CDsON() and TargetinRange(37) then
...
local function Serenity()
    ...
    if S.StrikeoftheWindlord:IsReady() and (RangeCount(40) < 3) and TargetinRange(37) then
    ...
    if S.FaelineStomp:IsReady() and TargetinRange(37) and
        (
    ...
    if S.FistsofFury:IsReady() and TargetinRange(37) and (not Player:IsChanneling(S.FistsofFury)) and
    ...
    if S.BlackoutKick:IsReady() and TargetinRange(37) and
        (ComboStrike(S.BlackoutKick) and RangeCount(40) == 3 and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
        return S.BlackoutKick:Cast()
    end
    -- spinning_crane_kick,if=combo_strike&(active_enemies>3|active_enemies>2&spinning_crane_kick.modifier>=2.3)
    if S.SpinningCraneKick:IsReady() and TargetinRange(37) and
        (ComboStrike(S.SpinningCraneKick) and (RangeCount(40) > 3 or RangeCount(40) > 2 and SCKModifier() >= 2.3)) then
        return S.SpinningCraneKick:Cast()
    end
    -- strike_of_the_windlord,if=active_enemies>=3
    if S.StrikeoftheWindlord:IsReady() and TargetinRange(37) and (RangeCount(40) >= 3) then
        return S.StrikeoftheWindlord:Cast()
    end
    -- spinning_crane_kick,if=combo_strike&active_enemies>1
    if S.SpinningCraneKick:IsReady() and TargetinRange(37) and (ComboStrike(S.SpinningCraneKick) and RangeCount(40) > 1) then
        return S.SpinningCraneKick:Cast()
    end
    -- whirling_dragon_punch,if=active_enemies>1
    if S.WhirlingDragonPunch:IsReady() and TargetinRange(37) and (RangeCount(40) > 1) then
    ...
    if S.BlackoutKick:IsReady() and TargetinRange(37) and
        (RangeCount(40) >= 3 and S.FistsofFury:CooldownDown() and S.ShadowboxingTreads:IsAvailable()) then
        return S.BlackoutKick:Cast()
    end
    -- rushing_jade_wind,if=!buff.rushing_jade_wind.up&active_enemies>=3
    if S.RushingJadeWind:IsReady() and TargetinRange(37) and
        (not Player:Buff(S.RushingJadeWindBuff) and RangeCount(40) >= 3
        ) then
        return S.RushingJadeWind:Cast()
    end
    -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains
    if S.RisingSunKick:IsReady() and TargetinRange(37) then
        return S.RisingSunKick:Cast()
    end
    ...
    if S.BlackoutKick:IsReady() and TargetinRange(37) and (ComboStrike(S.BlackoutKick)) then
        return S.BlackoutKick:Cast()
    end
    -- whirling_dragon_punch
    if S.WhirlingDragonPunch:IsReady() and TargetinRange(37) then
        return S.WhirlingDragonPunch:Cast()
    end
...
RangeCount()
    ...
    -- RangeCount(40) = Cache.EnemiesCount[10]
    -- RangeCount(40) = Cache.EnemiesCount[12]
...
-- print('BOK: ',ComboStrike(S.BlackoutKick))
    ...
    if not RubimRH.queuedSpell[1]:CooldownUp() or not TargetinRange(38) or not Player:AffectingCombat() then
        RubimRH.queuedSpell = { RubimRH.Spell[1].Empty, 0 }
    end
...
-- print(BossTarget:TimeToDie())
    ...
    and not Player:Buff(S.DampenHarmBuff) and not
        ...
        Player:NeedPanicHealing() and Player:HealthPercentage() < 50 or
            RubimRH.CDsON() and TargetinRange(37) and IsTanking and Player:HealthPercentage() < 80) then
        return S.TouchofKarma:Cast()
    end

    if S.DampenHarm:IsReady()     and RangeCount(40) >= 1 and not Player:Buff(S.FortifyingBrewBuff) and
        (Player:HealthPercentage() <= 40) then
        return S.DampenHarm:Cast()
    end

    if S.FortifyingBrew:IsReady() and RangeCount(40) >= 1 and not Player:Buff(S.DampenHarmBuff) and
        ( Player:HealthPercentage() <= 35) then
        return S.FortifyingBrew:Cast()
    end

    if S.DiffuseMagic:IsReady()     and RangeCount(40) >= 1 and not Player:Buff(S.DampenHarmBuff) and not Player:Buff(S.FortifyingBrewBuff) and
    ...
    if Target:Exists() and Player:CanAttack(Target) and not Target:IsDeadOrGhost() then
        ...
        and Target:AffectingCombat() and TargetinRange(37) then
        return S.autoattack:Cast()
        end

        -- Manually added: Force landing from FSK
        -- if (not Settings.Windwalker.IgnoreFSK) and Player:PrevGCD(1, S.FlyingSerpentKick) and TargetinRange(37) then
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------
        if not Player:AffectingCombat() then
            ...
            if S.SummonWhiteTigerStatue:IsReady() and RubimRH.CDsON() and TargetinRange(37) then
                return S.SummonWhiteTigerStatue:Cast()
            end
            -- expel_harm,if=chi<chi.max
            if S.ExpelHarm:IsReady() and (Player:Chi() < Player:ChiMax()) and TargetinRange(37) then
                return S.ExpelHarm:Cast()
            end
            -- chi_burst,if=!talent.faeline_stomp
            if S.ChiBurst:IsReady() and (not S.FaelineStomp:IsAvailable()) and Target:AffectingCombat() and TargetinRange(39) then
                return S.ChiBurst:Cast()
            end
            -- chi_wave
            if S.ChiWave:IsReady() and Target:AffectingCombat() and TargetinRange(39) then
    ...
    --call_action_list,name=opener,if=time<4&chi<5&!pet.xuen_the_white_tiger.active&!talent.serenity
        ...
        if HL.CombatTime() < 4 and Player:Chi() < 5 and not S.Serenity:IsAvailable() and (not XuenActive or not S.InvokeXuenTheWhiteTiger:IsAvailable()) and RangeCount(40) ==1 then
  
            -- expel_harm,if=talent.chi_burst.enabled&chi.max-chi>=3
            if S.ExpelHarm:IsReady() and S.ChiBurst:IsAvailable() and Player:ChiDeficit() >= 3 and TargetinRange(37) then
                return S.ExpelHarm:Cast()
            end
            ...
            if S.ChiWave:IsReady() and (Player:ChiDeficit() >= 2) and TargetinRange(39) and Target:AffectingCombat() then
                return S.ChiWave:Cast()
            end
            -- expel_harm
            if S.ExpelHarm:IsReady() and TargetinRange(37) then
                return S.ExpelHarm:Cast()
            end
    ...
    -- if S.TigerPalm:IsReady() and (TargetinRange(37) and S.SkyReach:IsAvailable() or TargetinRange(37)) then
    ...
    if not S.Serenity:IsAvailable() and S.StormEarthAndFire:IsAvailable() and RubimRH.CDsON() then
            ...
            if S.InvokeXuenTheWhiteTiger:IsReady() and TargetinRange(37) 
            and ((not VarHoldXuen) and S.BonedustBrew:IsAvailable() and S.BonedustBrew:CooldownRemains() <= 5 and
                    (RangeCount(40) < 3 and Player:Chi() >= 3 or RangeCount(40) >= 3 and Player:Chi() >= 2) or Target:TimeToDie() < 25) then
                return S.InvokeXuenTheWhiteTiger:Cast()
            end
            ...
            if S.StormEarthAndFire:IsReady()  and TargetinRange(37) and
                (
                S.BonedustBrew:IsAvailable() and
                    ...
                    Target:TimeToDie() < 30 and S.BonedustBrew:CooldownRemains() < 4 and Player:Chi() >= 4 or
                        ...
                        (not SCKMax()) and RangeCount(40) >= 3 and S.BonedustBrew:CooldownRemains() <= 2 and Player:Chi() >= 2) and
            ...
            -- if S.TouchofKarma:IsReady() and TargetinRange(37) and
            --     (
            --     (
            --         S.InvokeXuenTheWhiteTiger:IsAvailable() and
            ...
            if S.AncestralCall:IsReady()  and TargetinRange(37) and
            ...
            if S.Berserking:IsReady()  and TargetinRange(37) and
            ...
            if S.BagofTricks:IsReady()  and TargetinRange(37) and (not Player:Buff(S.StormEarthAndFireBuff)) then
                return S.BagofTricks:Cast()
            end
            -- lights_judgment
            if S.LightsJudgment:IsReady() and TargetinRange(37) then
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if RangeCount(40) >= 5 then
            -- if S.InvokeXuenTheWhiteTiger:IsReady() and TargetinRange(37) and RubimRH.CDsON() then
            --     return S.InvokeXuenTheWhiteTiger:Cast()
            -- end

            if S.TouchofDeath:IsReady() and TargetinRange(37) and RubimRH.CDsON() then
                return S.TouchofDeath:Cast()
            end
            
            if S.ExpelHarm:IsReady() and TargetinRange(37) and Player:Chi() < 6 and ComboStrike(S.ExpelHarm) then
                return S.ExpelHarm:Cast()
            end

            if S.TigerPalm:IsReady() and TargetinRange(37) and Player:Chi() < 5 and ComboStrike(S.TigerPalm) then
                return S.TigerPalm:Cast()
            end

            if S.StrikeoftheWindlord:IsReady() and TargetinRange(37) and RubimRH.CDsON() then
                return S.StrikeoftheWindlord:Cast()
            end
            ...
            if S.SpinningCraneKick:IsReady() and ComboStrike(S.SpinningCraneKick) and Player:Buff(S.DanceofChijiBuff) and
                TargetinRange(37) then
                return S.SpinningCraneKick:Cast()
            end


            -- Fists of Fury
            if S.FistsofFury:IsReady() and ComboStrike(S.FistsofFury) and TargetinRange(37) then
                return S.FistsofFury:Cast()
            end

            -- Faeline Stomp if targets don’t have Faeline Harmony debuff
            -- if S.FaelineStomp:IsReady() and ComboStrike(S.FaelineStomp) and TargetinRange(37) and not Target:Debuff(S.FaelineHarmony) then
            --   return Cast(S.FaelineStomp)
            -- end

            -- Whirling Dragon Punch
            if S.WhirlingDragonPunch:IsReady() and ComboStrike(S.WhirlingDragonPunch) and TargetinRange(37) then
                return S.WhirlingDragonPunch:Cast()
            end

            -- Rushing Jade Wind
            if S.RushingJadeWind:IsReady() and TargetinRange(37) then
                return S.RushingJadeWind:Cast()
            end

            -- Spinning Crane Kick
            if S.SpinningCraneKick:IsReady() and ComboStrike(S.SpinningCraneKick) and TargetinRange(37) then
                return S.SpinningCraneKick:Cast()
            end

            -- Blackout Kick
            if S.BlackoutKick:IsReady() and ComboStrike(S.BlackoutKick) and TargetinRange(37) then
                return S.BlackoutKick:Cast()
            end
            ...
            -- Faeline Stomp
            -- if S.FaelineStomp:IsReady() and ComboStrike(S.FaelineStomp) and TargetinRange(37) then
            --   return Cast(S.FaelineStomp)
            -- end

            -- Chi Burst
            if S.ChiBurst:IsReady() and ComboStrike(S.ChiBurst) and TargetinRange(39) then
                return S.ChiBurst:Cast()
            end

            -- Chi Wave
            if S.ChiWave:IsReady() and TargetinRange(39) then
                return S.ChiWave:Cast()
            end


            if S.ExpelHarm:IsReady() and ComboStrike(S.ExpelHarm) and TargetinRange(37) then
                return S.ExpelHarm:Cast()
            end

            if S.TigerPalm:IsReady() and ComboStrike(S.TigerPalm) and (S.SkyReach:IsAvailable() and TargetinRange(37) or TargetinRange(37)) then
                return S.TigerPalm:Cast()
            end
        ...
        -- 2-4 targets
        if RangeCount(40) >= 2 and RangeCount(40) < 5 then
            ...
            -- end
               ...
               -- Chi Burst
            if S.ChiBurst:IsReady() and TargetinRange(39) then
                return S.ChiBurst:Cast()
            end
        ...
        if RangeCount(40) == 1 then
            ...
            -- Blackout Kick with 3 stacks of Teachings of the Monastery
            if S.BlackoutKick:IsReady() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and TargetinRange(37) then
                return S.BlackoutKick:Cast()
            end
            -- Strike of the Windlord
            if S.StrikeoftheWindlord:IsReady() and TargetinRange(37) and RubimRH.CDsON() then
            ...
            -- Rising Sun Kick
            if S.RisingSunKick:IsReady() and ComboStrike(S.RisingSunKick) and TargetinRange(37) then
                return S.RisingSunKick:Cast()
            end
            ...
            -- Rushing Jade Wind
            if S.RushingJadeWind:IsReady() and ComboStrike(S.RushingJadeWind) and TargetinRange(37) then
                return S.RushingJadeWind:Cast()
            end
