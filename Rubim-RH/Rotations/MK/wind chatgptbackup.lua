...
-- print(BossTarget:TimeToDie())
 ...
 -- Opener for AOE (>= 4 targets)
 if HL.CombatTime() < 10 and Cache.EnemiesCount[10] >= 4 then
    ...
    if S.SummonWhiteTigerStatue:IsReady() and Target:IsInRange(40) then
        return S.SummonWhiteTigerStatue:Cast()
    end
    if S.InvokeXuenTheWhiteTiger:IsReady() then
        return S.InvokeXuenTheWhiteTiger:Cast()
    end
    if S.BonedustBrew:IsReady() and Target:IsInRange(10) then
        return S.BonedustBrew:Cast()
    end
    if S.StormEarthAndFire:IsReady() then
        return S.StormEarthAndFire:Cast()
    end
    if S.TigerPalm:IsReady() and Target:IsInRange(5) then
        return S.TigerPalm:Cast()
    end
    if S.StrikeoftheWindlord:IsReady() and Target:IsInRange(10) then
        return S.StrikeoftheWindlord:Cast()
    end
    if S.FistsofFury:IsReady() and Target:IsInRange(10) then
        return S.FistsofFury:Cast()
    end
    if S.TigerPalm:IsReady() and Target:IsInRange(5) then
        return S.TigerPalm:Cast()
    end
    if S.RisingSunKick:IsReady() and S.WhirlingDragonPunch:IsReady() and Target:IsInRange(10) then
        return S.RisingSunKick:Cast()
    end
    if S.WhirlingDragonPunch:IsReady() and Target:IsInRange(10) then
        return S.WhirlingDragonPunch:Cast()
    end
-- Opener for single target (<10s in combat)
elseif HL.CombatTime() < 10 and Cache.EnemiesCount[10] < 4 then
    if S.ExpelHarm:IsReady() then
        return S.ExpelHarm:Cast()
    end
    ...
    if S.FaelineStomp:IsReady() and Target:IsInRange(10) then
        return S.FaelineStomp:Cast()
    end
    if S.TigerPalm:IsReady() and Target:IsInRange(5) then
        return S.TigerPalm:Cast()
...
if S.RisingSunKick:IsReady() and Target:IsInRange(10) then
    return S.RisingSunKick:Cast()
end
end

-- Rest of the APL function here (previous priority list code)

if HL.CombatTime() < 10 then
    if not Player:Buff(S.ChiWave) then
        return S.ChiWave:Cast()
    end
    if S.ExpelHarm:IsReady() and Player:HealthPercentage() < 100 and not Player:Buff(S.ExpelHarm) then
        return S.ExpelHarm:Cast()
    end
end
...
if S.InvokeXuenTheWhiteTiger:IsReady() and Target:IsInRange(40) then
    return S.InvokeXuenTheWhiteTiger:Cast()
end

if S.BonedustBrew:IsReady() and Cache.EnemiesCount[10] >= 2 and Target:IsInRange(40) then
    return S.BonedustBrew:Cast()
end

if S.StormEarthAndFire:IsReady() and Target:IsInRange(40) then
    return S.StormEarthAndFire:Cast()
end

if S.TouchofDeath:IsReady() and Target:IsInRange(5) then
    return S.TouchofDeath:Cast()
end

if S.TouchofKarma:IsReady() and Player:HealthPercentage() < 70 then
    return S.TouchofKarma:Cast()
end

if Cache.EnemiesCount[10] >= 4 then
    -- AOE rotation for 4 or more targets
    if S.FaelineStomp:IsReady() and not Target:Debuff(S.FaelineHarmony) and Target:IsInRange(10) then
        return S.FaelineStomp:Cast()
    end
    if S.StrikeoftheWindlord:IsReady() and Target:IsInRange(10) then
        return S.StrikeoftheWindlord:Cast()
    end
    if S.SpinningCraneKick:IsReady() and Player:Buff(S.DanceofChijiBuff) and Target:IsInRange(10) then
        return S.SpinningCraneKick:Cast()
    end
    if S.FistsofFury:IsReady() and Target:IsInRange(10) then
        return S.FistsofFury:Cast()
    end
    if S.WhirlingDragonPunch:IsReady() and S.RisingSunKick:IsReady() and Target:IsInRange(10) then
        return S.WhirlingDragonPunch:Cast()
    end
    if S.RushingJadeWind:IsReady() and Target:IsInRange(10) then
        return S.RushingJadeWind:Cast()
    end
    if S.SpinningCraneKick:IsReady() and Target:IsInRange(10) then
        return S.SpinningCraneKick:Cast()
    end
    if S.BlackoutKick:IsReady() and Player:BuffStack(S.TeachingsoftheMonastery) < 3 and Target:IsInRange(5) then
        return S.BlackoutKick:Cast()
    end
    if S.RisingSunKick:IsReady() and Target:IsInRange(10) then
        return S.RisingSunKick:Cast()
...
elseif Cache.EnemiesCount[10] >= 2 then
-- AOE rotation for 2-3 targets
if S.FaelineStomp:IsReady() and not Target:Debuff(S.FaelineHarmony) and Target:IsInRange(10) then
    return S.FaelineStomp:Cast()
end
if S.StrikeoftheWindlord:IsReady() and Target:IsInRange(10) then
...
if S.SpinningCraneKick:IsReady() and Player:Buff(S.BonedustBrew) and Target:IsInRange(10) then
    return S.SpinningCraneKick:Cast()
end
if S.BlackoutKick:IsReady() and Player:BuffStack(S.TeachingsoftheMonastery) < 3 and Target:IsInRange(5) then
    return S.BlackoutKick:Cast()
...
if S.RisingSunKick:IsReady() and Player:Buff(S.XuensBattlegearCritBuff) and Target:IsInRange(10) then
    return S.RisingSunKick:Cast()
end
if S.FistsofFury:IsReady() and Target:IsInRange(10) then
    return S.FistsofFury:Cast()
