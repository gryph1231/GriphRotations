...
if inRange10>=2 and RubimRH.AoEON() then
  ...
  if RubimRH.CDsON() and Player:PrevGCD(1, S.Rampage) and aoecds10y and IsReady("Recklessness") and ((not S.AngerManagement:IsAvailable() and S.Avatar:CooldownRemains() < 1 and S.TitansTorment:IsAvailable()) or S.AngerManagement:IsAvailable() or not S.TitansTorment:IsAvailable()) then
    return S.Recklessness:Cast()
  end
  -- avatar,if=talent.titans_torment&(buff.enrage.up|talent.titanic_rage)|!talent.titans_torment
  if RubimRH.CDsON() and S.Recklessness:CooldownDown() and aoecds10y and IsReady("Avatar") and (S.TitansTorment:IsAvailable() and (EnrageUp or S.TitanicRage:IsAvailable()) or not S.TitansTorment:IsAvailable()) then
    return S.Avatar:Cast()
  end
  -- thunderous_roar,if=buff.enrage.up
  if RubimRH.CDsON() and S.OdynsFury:CooldownDown() and aoecds10y and IsReady("Thunderous Roar") and (EnrageUp) then
  ...
  if RubimRH.CDsON() and S.Recklessness:CooldownDown() and aoecds10y and IsReady("Odyn's Fury") and (Target:DebuffRemains(S.OdynsFuryDebuff) < 1 and (EnrageUp or S.TitanicRage:IsAvailable()) and S.Avatar:CooldownDown()) then
  ...
  if RubimRH.CDsON() and S.ThunderousRoar:CooldownDown() and aoecds10y and IsReady("Bladestorm") and (EnrageUp and S.Avatar:CooldownRemains() >= 9) and S.OdynsFury:CooldownRemains()>Player:GCD() and S.ThunderousRoar:CooldownRemains()>Player:GCD() then
