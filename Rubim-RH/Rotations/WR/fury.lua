...
if Player:AffectingCombat() and Player:CanAttack(Target) then
	if not IsCurrentSpell(6603) and Target:IsInRange(10) then
		...
		if IsUsableSpell('Execute') and Player:Buff(S.AshenJuggernautBuff) and Player:BuffP(S.EnrageBuff) and ExecuteCD == 0 and Target:IsInRange(8) then
		...
		if IsUsableSpell('Execute') and Player:BuffP(S.EnrageBuff) and ExecuteCD == 0 and Target:IsInRange(8) then
			return S.Execute:Cast()
		end
