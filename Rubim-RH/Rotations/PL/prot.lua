...
Intercession = Spell(391054),
Intercessionz = Spell(327193), -- moment of glory
};
...
wordofglorycast = (Player:HolyPower() >= 3 or Player:BuffP(S.DivinePurpose) or Player:BuffP(S.ShiningLight) or Player:Buff(S.BastionofLight))
    ...
    and (not Player:InArena() and not Player:InBattlegrounds()) then
        return I.healingpotz:Cast()
    end


	if S.Intercession:ID() ==  RubimRH.queuedSpell[1]:ID() and S.Intercession:IsReady() and UnitExists("mouseover") then
    	return S.Intercessionz:Cast() -- BIND brez to stoneform
	end
	
	if S.Intercession:ID() ==  RubimRH.queuedSpell[1]:ID() and (not S.Intercession:CooldownUp() or not UnitExists("mouseover") or not UnitIsDeadOrGhost("mouseover")) then
	...
	if S.lustAT:ID() ==  RubimRH.queuedSpell[1]:ID() and (Player:Debuff(S.lust1) or Player:Debuff(S.lust2) or Player:Debuff(S.lust3) or Player:Debuff(S.lust4) or Player:Debuff(S.lust5)) then
	  ...
	  if I.trink:IsReady() and Cache.EnemiesCount[5]>=1 and Target:IsInRange(8) and I.trink:IsEquipped() and RubimRH.CDsON() then
    ...
    if select(8,UnitCastingInfo("target")) == false and Target:CastPercentage() > math.random(43,87) and RubimRH.InterruptsON() and S.Rebuke:IsReady(8) and Player:AffectingCombat() then
    ...
    and Target:AffectingCombat() and Player:AffectingCombat() and Target:IsInRange(20) then
		...
		if S.ArdentDefender:IsReadyP() and Player:AffectingCombat() and not Player:Buff(S.GuardianofAncientKings) and (Player:NeedPanicHealing() or Player:NeedMajorHealing()) and Player:HealthPercentage() <= 35 and Cache.EnemiesCount[10]>=1 then
            return S.ArdentDefender:Cast()
        end


		if S.ArdentDefender:IsReadyP() and not Player:Buff(S.GuardianofAncientKings) and (Player:NeedPanicHealing() or Player:NeedMajorHealing()) and Player:HealthPercentage() <= 25 and (S.LayonHands:CooldownRemains()>1 or not Player:InArena()) and Cache.EnemiesCount[10]>=1 then
            return S.ArdentDefender:Cast()
        end
	
        if S.ArdentDefender:IsReadyP() and Cache.EnemiesCount[10]>=1 and Player:HealthPercentage() <= 60 and not Player:Buff(S.GuardianofAncientKings) and (Player:NeedPanicHealing() or Player:NeedMajorHealing()) then
            return S.ArdentDefender:Cast()
        end	
	
        if S.GuardianofAncientKings:IsReadyP() and (Player:NeedPanicHealing() and Player:HealthPercentage() <= 70 or Player:NeedMajorHealing() and Player:HealthPercentage()<55) and not Player:Buff(S.ArdentDefender) and not Player:Buff(S.DivineShield) and Cache.EnemiesCount[10]>=1 then
	...
	if S.LayonHands:IsReadyP() and Player:HealthPercentage()<= 25 and not Player:Debuff(S.Forbearance) and not Player:InArena() and Cache.EnemiesCount[30]>=1 then
		return S.LayonHands:Cast()
	end
		

			if S.DivineShield:IsReady() and Player:HealthPercentage()<= 25 and not Player:Debuff(S.Forbearance) and not Player:BuffP(S.ArdentDefender) and S.ArdentDefender:CooldownRemains()>1 and not Player:Buff(S.GuardianofAncientKings) and Cache.EnemiesCount[15]>=1 then
		return S.DivineShield:Cast()
	end


	if S.BastionofLight:IsReadyP() and Cache.EnemiesCount[8]>=1 and RubimRH.CDsON() then
        return S.BastionofLightz:Cast() 
    end
	

	
	if S.WordofGlory:IsReadyP() and Player:HealthPercentage() < 65 and wordofglorycast and Player:BuffRemains(S.ShieldoftheRighteousBuff)>5 then
        return S.WordofGlory:Cast() 
    end
	
	if S.WordofGlory:IsReadyP() and (Player:HealthPercentage() < 45 and wordofglorycast or Player:HealthPercentage() < 70 and Player:HolyPower() == 5 and Cache.EnemiesCount[20]==0) and Player:Buff(S.ShieldoftheRighteousBuff) then
        return S.WordofGlory:Cast() 
    end	
	
	if S.BlessingofProtection:IsReadyP() and Player:HealthPercentage()<= 20 and not Player:DebuffP(S.Forbearance) and S.DivineShield:CooldownRemains() > Player:GCD() and not Player:BuffP(S.ArdentDefender) and not Player:Buff(S.GuardianofAncientKings) and S.LayonHands:CooldownRemains()>Player:GCD() and Cache.EnemiesCount[10]>=1 then
...
consecrationdrop = (Player:CanAttack(Target) and Target:IsInRange(8) and (Player:MovingFor() <= Player:GCD() or allMobsinRange(10) and tarSpeed == 0) or Cache.EnemiesCount[5]>=2 )
...
-- shield_of_the_righteous,if=debuff.judgment.up&(debuff.vengeful_shock.up|!conduit.vengeful_shock.enabled)
    ...
    if S.ShieldoftheRighteous:IsReady() and Target:IsInRange(5) and (Target:Debuff(S.JudgmentDebuff) or Player:HealthPercentage()<90 and not Player:Buff(S.ShieldoftheRighteousBuff) and (Player:NeedMajorHealing() or Player:NeedPanicHealing())) 
...
-- shield_of_the_righteous,if=holy_power=5|buff.holy_avenger.up|holy_power=4&talent.sanctified_wrath.enabled&buff.avenging_wrath.up
    ...
    if S.ShieldoftheRighteous:IsReady() and Target:IsInRange(5) and (Player:HolyPower() == 5 or Player:BuffP(S.HolyAvenger) or Player:HolyPower()==4 and S.SanctifiedWrath:IsAvailable() and Player:BuffP(S.AvengingWrathBuff)) then
        return S.ShieldoftheRighteous:Cast()
    end	
...
-- judgment,target_if=min:debuff.judgment.remains,if=charges=2|!talent.crusaders_judgment.enabled
    ...
    if S.Judgment:IsReadyP() and Target:IsInRange(30) and (Target:DebuffRemains(S.JudgmentDebuff)<Player:GCD() and S.Judgment:ChargesFractional()>= 1.9 or not S.CrusadersJudgment:IsAvailable()) then
...
-- divine_toll
    ...
    if S.DivineToll:IsReadyP() and Cache.EnemiesCount[16]>=3 and Target:IsInRange(30) and RubimRH.CDsON() and (Player:BuffP(S.AvengingWrathBuff) or S.AvengingWrath:CooldownRemains()>0) then
...
-- hammer_of_wrath
	...
	if S.HammerofWrath:IsReadyP() and Target:IsInRange(30) and (Target:HealthPercentage()<=20 or Player:BuffP(S.AvengingWrathBuff)) then
...
-- divine_toll
    ...
    if S.DivineToll:IsReadyP() and Target:IsInRange(30) and RubimRH.CDsON() and (Player:BuffP(S.AvengingWrathBuff) or S.AvengingWrath:CooldownRemains()>0) then
        return S.DivineToll:Cast()
    end
...
-- hammer_of_the_righteous,if=charges=2
    if S.HammeroftheRighteous:IsReadyP() and Cache.EnemiesCount[12]>=1 and S.HammeroftheRighteous:ChargesFractional()>=2.9 then
...
-- consecration
    ...
    if not Player:AffectingCombat() then
		...
		if S.WordofGlory:IsCastable() and Player:HealthPercentage() < 75  and (Player:HolyPower() >= 3 or Player:BuffP(S.DivinePurpose) or Player:BuffP(S.ShiningLight)) then
        return S.WordofGlory:Cast()
    end
