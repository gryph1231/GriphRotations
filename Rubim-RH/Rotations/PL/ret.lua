...
S.Intercession.TextureSpellID = { 215661 } -- justicars vengeance
S.Exorcism.TextureSpellID = { 183218 } -- hand of hindrance
S.RadiantDecree.TextureSpellID = { 205191 } -- eye for eye
...
local function APL()
    ...
    ComputeTimeToHPG()
        ...
        consecrationdrop = Target:IsInRange(8) and (Player:MovingFor() <= Player:GCD() or (allMobsinRange(10) and tarSpeed == 0))
...
if S.RadiantDecree:IsAvailable() then 
    ...
    Cooldowns = function()
        ...
        and (HL.CombatTime()>5 or IsEquippedItem("The Magistrates Judgment")) or Player:HolyPower()>=2 and IsEquippedItem("Vanguard's Momentum") and S.FinalReckoning:IsAvailable() or S.HolyAvenger:IsAvailable() and S.HolyAvenger:CooldownUp())
        and (not S.Seraphim:IsAvailable() or not S.FinalReckoning:IsAvailable() or S.Seraphim:CooldownRemains()>0)
        )
        then 
        ...
        if S.Crusade:IsReadyP() and (Player:HolyPower()>=5 and HL.CombatTime()<5 or Player:HolyPower()>=4 and HL.CombatTime()>5) then 
  ...
  -- ashen_hallow
        ...
        and (Player:BuffP(S.AvengingWrath) or S.Crusade:IsAvailable() and (S.Crusade:CooldownUp() or Player:Buff(S.Crusade)) or Target:TimeToDie()<20) then
        ...
        (
            (Player:HolyPower()>=4 and HL.CombatTime()<8 or Player:HolyPower()>=3 and (HL.CombatTime()>=8 or Cache.EnemiesCount[12]>=2 and covenantID==1)) 
        and (S.AvengingWrath:CooldownRemains()>Player:GCD() or not S.Crusade:CooldownUp() and (not Player:Buff(S.Crusade) or Player:BuffStackP(S.Crusade)>=10))
        and (TimeToHPG>0 or Player:HolyPower()==5) and (not S.Seraphim:IsAvailable() or Player:Buff(S.Seraphim))
        and (not Player:BuffP(S.AvengingWrath) or Player:HolyPower()==5 or not S.HammerofWrath:CooldownUp() or Cache.EnemiesCount[12]>=2 and covenantID==1)) then
    ...
    Finishers = function()
        ...
        if (true) then
        VarDsCastable = ((Cache.EnemiesCount[12] >= 2 and I.tempest:IsEquipped() or Cache.EnemiesCount[8] >= 2 and not I.tempest:IsEquipped())
        or (Player:BuffP(S.EmpyreanPowerBuffTalent)
        and not Target:Debuff(S.JudgmentDebuff) 
        and not Player:BuffP(S.DivinePurposeBuff) 
        or (Cache.EnemiesCount[12] >= 2 and I.tempest:IsEquipped() or Cache.EnemiesCount[8] >= 2 and not I.tempest:IsEquipped())
        ...
        if IsUsableSpell('Radiant Decree') and Player:HolyPower()>=3 and S.RadiantDecree:CooldownUp() and Target:IsInRange(6)
        and ((not Player:Buff(S.Crusade) or S.Crusade:CooldownRemainsP()>5 or Player:BuffStackP(S.Crusade)>=3 or S.AvengingWrath:CooldownRemains()>5))
        ...
        and (not RubimRH.CDsON() or not S.ExecutionSentence:IsAvailable() or S.ExecutionSentence:CooldownRemainsP()>Player:GCD()*3 or (Cache.EnemiesCount[12] >= 3 and I.tempest:IsEquipped() or Cache.EnemiesCount[8] >= 3 and not I.tempest:IsEquipped())) 
        or (Cache.EnemiesCount[12] >= 2 and I.tempest:IsEquipped() or Cache.EnemiesCount[8] >= 2 and not I.tempest:IsEquipped()) and (S.HolyAvenger:IsAvailable() and S.HolyAvenger:CooldownRemainsP()<Player:GCD()*3 or Player:BuffP(S.Crusade) and Player:BuffStackP(S.Crusade)<10)) then
    ...
    Generators = function()
        ...
        --burst AOE
            ...
            if S.DivineToll:IsCastable() and RubimRH.CDsON() and Player:HolyPower() <=2 and Cache.EnemiesCount[14] >= 3 
        ...
        --&(cooldown.avenging_wrath.remains>15|cooldown.crusade.remains>15|fight_remains<8)
            ...
            and not S.FinalReckoning:IsAvailable() and (not S.ExecutionSentence:IsAvailable() or Target:TimeToDie()<8 or Cache.EnemiesCount[14] >= 5) 
