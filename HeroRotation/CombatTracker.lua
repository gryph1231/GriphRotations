...
function HR.getRealTimeDMG(UNIT)
    local total, Hits, phys, magic, swing = 0, 0, 0, 0, 0
    local combatTime = HR.CombatTime(UNIT)
    local GUID = UnitGUID(UNIT)
...
function HR.getRealTimeDPS(UNIT)
    local total, Hits, phys, magic, swing = 0, 0, 0, 0, 0
    local combatTime = HR.CombatTime(UNIT)
    local GUID = UnitGUID(UNIT)
...
function HR.getHealSpellAmount(UNIT, SPELL, timer)
...
--[[ Get Heal Taken ]]
function HR.getAbsorb(unit, spellID)
    local GUID = UnitGUID(unit)
...
--[[function HR.TimeToDie(unit)
    local ttd = 0
    local DMG, Hits = HR.getDMG(unit)
...
function HR.TimeToDieMagicX(UNIT, X)
    if not UNIT then 
		UNIT = "target" 
	end
...
function HR.TimeToDieMagic(UNIT)
    if not UNIT then 
		UNIT = "target" 
	end
...
function HR.SpellDamage(_, unit, spellID)
    local GUID = UnitGUID(unit)
    return Data[GUID] and Data[GUID][spellID] or 0
end

HR.Listener:Add('HR_Events', 'COMBAT_LOG_EVENT_UNFILTERED', function(...)
...
HR.Listener:Add('HR_Events', 'PLAYER_REGEN_ENABLED', function()
    wipe(Data)
end)

HR.Listener:Add('HR_Events', 'PLAYER_REGEN_DISABLED', function()
    wipe(Data)
end)

--- ========================== INCOMING ============================
-- DS 
function HR.LastIncDMG(unit, seconds)
...
HR.last5secs = 0
function HR.incdmg5secs(unit)
    if UnitExists(unit) then
        local pDMG = select(1, HR.getDMG(unit))
        HR.last5secs = pMDG
    end
    return 0
end    

function HR.incdmg(unit)
    if UnitExists(unit) then
        local pDMG = select(1, HR.getDMG(unit))
        return pDMG or 0
    end
...
function HR.incdmgphys(unit)
    if UnitExists(unit) then
        local pDMG = select(3, HR.getDMG(unit))
        return pDMG
    end
    return 0
end

function HR.incdmgswing(unit)
    if UnitExists(unit) then
        local mDMG = select(5, HR.getDMG(unit))
        return mDMG
    end
    return 0
end

function HR.lastSwing(unit)
...
function HR.incdmgmagic(unit)
    if UnitExists(unit) then
        local mDMG = select(4, HR.getDMG(unit))
        return mDMG
    end
    return 0
end

function HR.lastDmg5()
    if UnitExists("player") then
        local pDMG = (select(6, HR.getDMG("player")) * 100) / UnitHealthMax("Player")
        return pDMG
    end
    return 0
end
...
function HR.LossOfControlCreate(locType, name, ...)
...
function HR.LossOfControlRemove(locType, name)
    if name then 
...
function HR.LossOfControlGet(locType, name)
    local result = 0
    if not LossOfControl[locType] then
...
-- function HR.LossOfControlUpdate()
    ...
    -- if not isValidType then 
        -- for name in pairs(LossOfControl) do 
            -- if type(name) ~= "table" and HR.LossOfControlGet(name) > 0 then
...
-- do 
    -- --- PvP Trinket:
    -- HR.LossOfControlCreate("DISARM")
    -- HR.LossOfControlCreate("INCAPACITATE")
    -- HR.LossOfControlCreate("DISORIENT")
    -- HR.LossOfControlCreate("FREEZE")        
    -- HR.LossOfControlCreate("SILENCE")
    -- HR.LossOfControlCreate("POSSESS")    
    -- HR.LossOfControlCreate("SAP")    
    -- HR.LossOfControlCreate("CYCLONE")
    -- HR.LossOfControlCreate("BANISH")
    -- HR.LossOfControlCreate("PACIFYSILENCE")
    -- --- Dworf|DarkIronDwarf
    -- HR.LossOfControlCreate("POLYMORPH")    
    -- HR.LossOfControlCreate("SLEEP")
    -- HR.LossOfControlCreate("SHACKLE_UNDEAD")
    -- --- Scourge + WR Berserk Rage + DK Lichborne
    -- HR.LossOfControlCreate("FEAR")    
    -- HR.LossOfControlCreate("HORROR")    
    -- --- Scourge
    -- HR.LossOfControlCreate("CHARM")        
    -- --- Gnome and any freedom effects 
    -- HR.LossOfControlCreate("ROOT")        
    -- HR.LossOfControlCreate("SNARE")
    -- --- Human + DK Icebound|Lichborne
    -- HR.LossOfControlCreate("STUN")
-- end 

-- HR.Listener:Add('HR_Events', "LOSS_OF_CONTROL_UPDATE", HR.LossOfControlUpdate)
-- HR.Listener:Add('HR_Events', "LOSS_OF_CONTROL_ADDED", HR.LossOfControlUpdate)
