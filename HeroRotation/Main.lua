...
HR.CurrentAction = nil
HR.Cache = {}
HR.Cache.Persistent = {}
HR.Cache.Temp = {}
HR.GlobalTickCount = 0
HR.IgnoreGeneralPvP = {}
--HR.disabledSpellsCD = { HR.Spell[1].Empty }
HR.CustomTrinket = {}
HR.List = {}
HR.List.Cleave = {}
HR.List.Stuns = {}
HR.List.InterruptsCC = {}
HR.List.Interrupts = {}
HR.List.Snare = {}
--- ============================ CONTENT ============================
--- ======= BINDINGS =======
...
function HR.MainFrame:ResizeUI(Multiplier)
  local FramesToResize = {
    ...
    { HR.SmallIconFrame.Icon[1], HR.GUISettings.General.BlackBorderIcon and 30 or 32,
      HR.GUISettings.General.BlackBorderIcon and 30 or 32 },
    { HR.SmallIconFrame.Icon[2], HR.GUISettings.General.BlackBorderIcon and 30 or 32,
      HR.GUISettings.General.BlackBorderIcon and 30 or 32 },
  ...
  HR.SuggestedIconFrame:SetPoint("BOTTOM", HR.MainIconFrame, "LEFT", -HR.LeftIconFrame:GetWidth() / 2,
    HR.LeftIconFrame:GetHeight() / 2 + (HR.GUISettings.General.BlackBorderIcon and 3 * Multiplier or 4 * Multiplier));
  HR.RightSuggestedIconFrame:SetPoint("BOTTOM", HR.MainIconFrame, "RIGHT", HR.LeftIconFrame:GetWidth() / 2,
    HR.LeftIconFrame:GetHeight() / 2 + (HR.GUISettings.General.BlackBorderIcon and 3 * Multiplier or 4 * Multiplier)); -- todo matt fix this location
...
function HR.MainFrame:ResizeButtons(Multiplier)
  ...
  for i = 1, 2 do
    HR.ToggleIconFrame.Button[i]:SetPoint("CENTER", HR.ToggleIconFrame, "CENTER",
      HR.ToggleIconFrame.Button[i]:GetWidth() * (i - 1) + i, 0);
  end
  ...
  if Event == "ADDON_LOADED" then
    if Arg1 == "HeroRotation" then
      ...
      if HeroRotationDB and type(HeroRotationDB.IconFramePos) == "table" and #HeroRotationDB.IconFramePos == 5 then
        HR.MainFrame:SetPoint(HeroRotationDB.IconFramePos[1], _G[HeroRotationDB.IconFramePos[2]],
          HeroRotationDB.IconFramePos[3], HeroRotationDB.IconFramePos[4], HeroRotationDB.IconFramePos[5]);
      else
      ...
      if CP_General then
        CreatePanelOption("Slider", CP_General, "General.SetAlpha", { 0, 1, 0.05 }, "Addon Alpha",
          "Change the addon's alpha setting.");
        CreatePanelOption("Button", CP_General, "ButtonMove", "Lock/Unlock", "Enable the moving of the frames.",
          function() HR.MainFrame:ToggleLock(); end);
        CreatePanelOption("Button", CP_General, "ButtonReset", "Reset Buttons", "Resets the anchor of buttons.",
          function() HR.ToggleIconFrame:ResetAnchor(); end);
      end
      local CP_Scaling = GUI.GetPanelByName("Scaling")
      if CP_Scaling then
        CreatePanelOption("Slider", CP_Scaling, "Scaling.ScaleUI", { 0.5, 5, 0.1 }, "UI Scale", "Scale of the Icons.",
          function(value) HR.MainFrame:ResizeUI(value); end);
        CreatePanelOption("Slider", CP_Scaling, "Scaling.ScaleButtons", { 0.5, 5, 0.1 }, "Buttons Scale",
          "Scale of the Buttons.", function(value) HR.MainFrame:ResizeButtons(value); end);
        CreatePanelOption("Slider", CP_Scaling, "Scaling.ScaleHotkey", { 0.5, 5, 0.1 }, "Hotkey Scale",
          "Scale of the Hotkeys.");
      end
...
HR.Rotation = {}
HR.Rotation.APLs = {}
HR.Rotation.PASSIVEs = {}
HR.Rotation.PvP = {}
HR.CreateConfig = {}


function HR.SetConfig (Spec, APL)
    HR.CreateConfig[Spec] = APL;
end

function HR.Rotation.SetAPL (Spec, APL)
    HR.Rotation.APLs[Spec] = APL;
end

function HR.Rotation.SetPASSIVE (Spec, APL)
    HR.Rotation.PASSIVEs[Spec] = APL;
end

function HR.Rotation.SetPvP (Spec, APL)
    HR.Rotation.PvP[Spec] = APL;
end
--- ======= MAIN =======
    ...
    if SpecID == nil then
      ...
      if LatestSpecIDChecked ~= SpecID then
        if EnabledRotation[SpecID] and HR.APLs[SpecID] then
          ...
          HR.Print("No Rotation found for this class/spec (SpecID: " ..
            SpecID ..
...
function HR.playSoundR(soundID)
...
function HR.Pulse()
  if GetTime() > HR.Timer.Pulse and HR.Locked() then
    ...
    c = Cache.Persistent.Player.Class[1] or 0
    if c == 0 then
      return
    end
  end
...
function HR.Ready()
  ...
  if HR.GUISettings.General.ShowWhileMounted then
    ...
    AreWeReady = not Player:IsDeadOrGhost() and not Player:IsMounted() and not Player:IsInVehicle() and
...
updateClassVariables:SetScript("OnEvent", function(self, event, ...)
    HR.playerSpec = Cache.Persistent.Player.Spec[1] or 0
    if HR.playerSpec == 0 then
        return
    end

    if HeroRotationDB[HR.playerSpec] == nil then
        return
    end

    --if HR.playerSpec ~= 0 then
    --        Player:RegisterListenedSpells(HR.playerSpec)
    --    end
    HR.config = {}
    HR.allSpells = {}
    if HR.playerSpec ~= 0 then
        HR.config = HeroRotationDB[HR.playerSpec]
        for pos, spell in pairs(HR.Spell[HR.playerSpec]) do
            if spell:IsAvailable() then
                table.insert(HR.allSpells, spell)
            end
        end
    end
    HR.useCD = false or HeroRotationDB[HR.playerSpec].cooldown
end)
...
function updateIcon:onUpdate(sinceLastUpdate)
  ...
  if (self.sinceLastUpdate >= 0.066) then
    ...
    if UnitInVehicle("Player") then
      HR.stIcon.texture:SetTexture(236254)
    elseif Player:IsDeadOrGhost() then
      HR.stIcon.texture:SetTexture(236399)
    elseif Player:IsMounted() or
        (select(3, UnitClass("player")) == 11 and (GetShapeshiftForm() == 3 or GetShapeshiftForm() == 5)) then
      HR.stIcon.texture:SetTexture(975744)
    elseif Player:IsChanneling() or Player:IsCasting() then
      HR.stIcon.texture:SetTexture("Interface\\Addons\\HeroRotation\\Textures\\channel.tga", true)
    elseif ACTIVE_CHAT_EDIT_BOX ~= nil then
      HR.stIcon.texture:SetTexture(236254)
    elseif SpellIsTargeting() then
      HR.stIcon.texture:SetTexture(236353)
    elseif _G.LootFrame:IsShown() then
      HR.stIcon.texture:SetTexture(975746)
    elseif not HR.MainIconFrame:IsVisible() then
      HR.stIcon.texture:SetTexture(nil)
    else
      HR.stIcon.texture:SetTexture(tonumber(HR.MainIconFrame.Texture:GetTexture(Texture)))
