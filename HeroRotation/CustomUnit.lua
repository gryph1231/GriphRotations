...
function Unit:LastDamage3Seconds()
    local IncomingDPS = (HR.getLastDamage() / UnitHealthMax("player")) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:IncDmgPercentage()
    local unit = self.UnitID
    local IncomingDPS = (HR.getDMG(unit) / UnitHealthMax(unit)) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:IncDmgSwing()
    local unit = self.UnitID
    local IncomingDPS = (HR.incdmgswing(unit) / UnitHealthMax(unit)) * 100
    return (math.floor((IncomingDPS * ((100) + 0.5)) / (100)))
end

function Unit:LastSwinged()
    local unit = self.UnitID
    return HR.lastSwing(unit)
end

-- Minor Defensive Usage (<= 1 Min CDs)
HR.MinorHealingThreshold = 2
function Unit:NeedMinorHealing()
    return (self:IncDmgPercentage() > HR.MinorHealingThreshold or self:HealthPercentage() <= 85)
end

-- Major Defensive Usage (<= 3 Min CDs)
HR.MajorHealingThreshold = 5
function Unit:NeedMajorHealing()
    return (self:IncDmgPercentage() > HR.MajorHealingThreshold or self:HealthPercentage() <= 60)
end

-- Panic Defensive Usage (> 3 Min CDs)
HR.PanicHealingThreshold = 10
function Unit:NeedPanicHealing()
    return (self:IncDmgPercentage() > HR.PanicHealingThreshold or self:HealthPercentage() <= 40)
end
...
-- function randomGenerator(option)
    -- if GetTime() - randomTimer >= 1 then
        -- randomInterrupt = math.random(HR.db.profile.mainOption.minInterruptValue, HR.db.profile.mainOption.maxInterruptValue)
...
function HR.UNITBoss(unitID)
    return HR.UNITLevel(unitID) == -1 or UnitEffectiveLevel(unitID) == -1 or UnitIsQuestBoss(unitID) or UnitIsBoss(unitID) or false 
end 
...
HR.Listener:Add('HR_Events', 'GROUP_ROSTER_UPDATE', function(...)
    CacheOtherTank()
end)
...
function HR.PetSpellInRange(id, unit)
    if not unit then 
	    unit = "target" 
	end 
    local slot = oPetSlots[HR.playerSpec] and oPetSlots[HR.playerSpec][id]
    return (slot and slot > 0 and IsActionInRange(slot, unit)) or false
end 

function HR.PetAoE(spellID, stop)
        ...
        for reference, unit in pairs(UnitPlates) do
            if type(spellID) == "table" then
                for k, v in pairs(spellID) do
                    if HR.PetSpellInRange(v, unit) then
                        total = total + 1  
                        break
                    end
                end
            elseif HR.PetSpellInRange(spellID, unit) then 
                total = total + 1                                            
            end  
...
local function UpdatePetSlots()
    ...
    for k, v in pairs(oPetSlots[HR.playerSpec]) do            
        if v == 0 then 
            for i = 1, 120 do 
                actionType, id, subType = GetActionInfo(i)
                if subType == "pet" and k == id then 
                    oPetSlots[HR.playerSpec][k] = i 
                    break 
                end 
                if i == 120 then 
                    display_error = true
...
HR.Listener:Add('PetSlots_Events', "UNIT_PET", function(...)
        if oPetSlots[HR.playerSpec] and  ... == "player" and (PetHasActionBar() or GetPetActionsUsable()) and HL.GetTime() ~= PetEvent_timestamp then     
            for k, v in pairs(oPetSlots[HR.playerSpec]) do
                if v == 0 then 
                    UpdatePetSlots()
                    break
...
HR.Listener:Add('PetSlots_Events', "ACTIONBAR_SLOT_CHANGED", function(...)
        if oPetSlots[HR.playerSpec] and (PetHasActionBar() or GetPetActionsUsable()) and HL.GetTime() ~= PetEvent_timestamp then
            for k, v in pairs(oPetSlots[HR.playerSpec]) do
                if v == 0 or v == ... then 
                    oPetSlots[HR.playerSpec][k] = 0
                    UseUpdate = true 
                end
            end         
            if UseUpdate then 
...
-- Enter game 
if HR.playerSpec and oPetSlots[HR.playerSpec] then 
    UpdatePetSlots()
end 
...
-- HR.Listener:Add('PvP_Events_Logs', "COMBAT_LOG_EVENT_UNFILTERED", function()
...
-- HR.Listener:Add('PvP_Events_UI', "UI_ERROR_MESSAGE", function(...)
