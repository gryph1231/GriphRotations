...
local function Precombat()
  ...
  -- snapshot_stats
  -- moonkin_form
  if (not Player:IsMounted() and not GetShapeshiftForm() == 3 and not GetShapeshiftForm() == 5) then 
    return Cast(S.MoonkinForm) 
    end
...
local function St()
  ...
  -- variable,name=save_for_ca_inc,value=cooldown.ca_inc.remains>10+5*covenant.venthyr|!variable.convoke_desync&covenant.night_fae|druid.no_cds
  VarSaveForCAInc = (CaInc:CooldownRemains() > 10 + 5 * num(CovenantID == 2) or (not VarConvokeDesync) and CovenantID == 3 or not CDsON())
...
local function APL()
  ...
  if not Player:IsMounted() and not GetShapeshiftForm() == 3 and S.MoonkinForm:IsCastable() then 
    return Cast(S.MoonkinForm) 
    end
  ...
  if Everyone.TargetIsValid() then
    ...
    VarCDCondition = (FightRemains > 15 and (CovenantID ~= 2 or S.Berserking:CooldownUp() or not Player:Race() == "Troll") and ((not S.FuryofElune:IsAvailable()) or CovenantID == 2 and S.FuryofElune:CooldownRemains() < 11 + 5 * num(SinfulHysteriaEquipped) or CovenantID ~= 2 and S.FuryofElune:CooldownRemains() < CaInc:BaseDuration() + 12 * num(PAPEquipped) - 12) and ((I.EmpyrealOrdinance:IsEquipped() and (CovenantID ~= 2 or I.EmpyrealOrdinance:CooldownRemains() < 167 + (5 * num(SinfulHysteriaEquipped))) and I.EmpyrealOrdinance:CooldownDown()) or I.SoullettingRuby:IsEquipped() and (CovenantID ~= 3 or I.SoullettingRuby:CooldownRemains() < 114) or I.InscrutableQuantumDevice:IsEquippedAndReady() or I.ShadowedOrbofTorment:IsEquipped() and I.ShadowedOrbofTorment:CooldownRemains() > 40 or (I.TheFirstSigil:IsEquipped() and (I.TheFirstSigil:CooldownRemains() < 20 or FightRemains > CaInc:BaseDuration() + 20)) or ((VarOnUseTrinket == 1 or VarOnUseTrinket == 3) and (trinket1:IsReady() or trinket1:CooldownRemains() > FightRemains - 10) or VarOnUseTrinket == 2 and (trinket2:IsReady() or trinket2:CooldownRemains() > FightRemains - 10) or VarOnUseTrinket == 0)))
    ...
    if (Settings.Commons.Enabled.Trinkets) then
      ...
      -- use_item,name=scars_of_fraternal_strife,if=(!buff.scars_of_fraternal_strife_4.up|buff.scars_of_fraternal_strife_4.up&buff.ca_inc.remains>20)&(cooldown.inscrutable_quantum_device.remains>20|!equipped.inscrutable_quantum_device)|fight_remains<33
      if I.ScarsofFraternalStrife:IsEquippedAndReady() and ((Player:BuffDown(S.ScarsofFraternalStrifeBuff4) or Player:BuffUp(S.ScarsofFraternalStrifeBuff4) and Player:BuffRemains(CaInc) > 20) and (I.InscrutableQuantumDevice:CooldownRemains() > 20 or not I.InscrutableQuantumDevice:IsEquipped()) or FightRemains < 33) then
        if Cast(I.ScarsofFraternalStrife, nil, Settings.Commons.DisplayStyle.Trinkets) then return "scars_of_fraternal_strife main 9"; end
      end
      -- variable,name=iqd_condition,value=buff.ca_inc.remains>15&(!runeforge.sinful_hysteria|buff.ravenous_frenzy.remains<=15.5+(3.5*buff.bloodlust.up)+(4.5-3.5*buff.bloodlust.up)*equipped.instructors_divine_bell+4.5*(equipped.the_first_sigil&cooldown.the_first_sigil_367241.remains<=20|equipped.scars_of_fraternal_strife&buff.scars_of_fraternal_strife_4.up))&buff.ravenous_frenzy.up)|fight_remains<25|equipped.empyreal_ordnance&cooldown.empyreal_ordnance.remains
      VarIQDCondition = (Player:BuffRemains(CaInc) > 15 and ((not SinfulHysteriaEquipped) or Player:BuffRemains(S.RavenousFrenzyBuff) <= 15.5 + (3.5 * num(Player:BloodlustUp())) + (4.5 - 3.5 * num(Player:BloodlustUp())) * num(I.InstructorsDivineBell:IsEquipped()) + 4.5 * num(I.TheFirstSigil:IsEquipped() and I.TheFirstSigil:CooldownRemains() <= 20 or I.ScarsofFraternalStrife:IsEquipped() and Player:BuffUp(S.ScarsofFraternalStrifeBuff4)) and Player:BuffUp(S.RavenousFrenzyBuff)) or FightRemains < 25 or I.EmpyrealOrdinance:IsEquipped() and not I.EmpyrealOrdinance:CooldownUp())
      ...
      -- use_item,name=shadowed_orb_of_torment,if=(!covenant.venthyr|(cooldown.ca_inc.ready&(cooldown.berserking.remains<2|!race.troll))&variable.thrill_seeker_wait)&(!covenant.night_fae|cooldown.convoke_the_spirits.remains<20)&dot.sunfire.ticking&(dot.stellar_flare.ticking|!talent.stellar_flare.enabled|spell_targets.starfire>3)&dot.moonfire.ticking&!equipped.inscrutable_quantum_device|equipped.inscrutable_quantum_device&cooldown.inscrutable_quantum_device.remains>30&!buff.ca_inc.up|fight_remains<40
      if I.ShadowedOrbofTorment:IsEquippedAndReady() and ((CovenantID ~= 2 or (CaInc:CooldownUp() and (S.Berserking:CooldownRemains() < 2 or not Player:Race() == "Troll")) and VarThrillSeekerWait) and (CovenantID ~= 3 or S.ConvoketheSpirits:CooldownRemains() < 20) and Target:DebuffUp(S.SunfireDebuff) and (Target:DebuffUp(S.StellarFlareDebuff) or (not S.StellarFlare:IsAvailable()) or EnemiesCount8ySplash > 3) and Target:DebuffUp(S.MoonfireDebuff) and (not I.InscrutableQuantumDevice:IsEquipped()) or I.InscrutableQuantumDevice:IsEquipped() and I.InscrutableQuantumDevice:CooldownRemains() > 30 and Player:BuffDown(CaInc) or FightRemains < 40) then
      ...
      --end
      -- use_items,slots=trinket2,if=variable.on_use_trinket!=2&!trinket.1.ready_cooldown|variable.on_use_trinket=2&(covenant.venthyr&buff.ca_inc.up&(buff.ravenous_frenzy.remains+(4.5*runeforge.sinful_hysteria)<=trinket.2.proc.any.duration&buff.ravenous_frenzy.up|buff.ravenous_frenzy_sinful_hysteria.up)|(cooldown.ca_inc.remains+15>trinket.2.cooldown.duration|(!runeforge.primordial_arcanic_pulsar|buff.ca_inc.remains>6))&(!covenant.venthyr|!buff.ravenous_frenzy.up&cooldown.ravenous_frenzy.remains)&(!covenant.night_fae|trinket.2.cooldown.duration>cooldown.convoke_the_spirits.remains|variable.convoke_desync&trinket.2.cooldown.duration>cooldown.ca_inc.remains)&!covenant.kyrian|covenant.night_fae&variable.convoke_desync&cooldown.convoke_the_spirits.up&!cooldown.ca_inc.up&(buff.eclipse_lunar.remains>5|buff.eclipse_solar.remains>5)|buff.kindred_empowerment_energize.up)|fight_remains<20|variable.on_use_trinket=0
      -- Temporarily held until we can get trinket.x.proc.any.duration figured out
  ...
  (select(3, UnitClass("player")) == 11 and (GetShapeshiftForm() == 3 or GetShapeshiftForm() == 5))) 
  then return HR.Cast(S.mounted)
